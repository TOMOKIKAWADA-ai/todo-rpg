...
function reducer(state, a) {
  switch (a.type) {
    case ACT.ADD_BLOCK: {
      ...
      return {
        ...state,
        blocks: [...state.blocks, { id: crypto.randomUUID(), tasks, hp: tasks.length, max: tasks.length, completed: false }],
      };
    }
    case ACT.ADD_TASK: {
...
export default function TodoRPGApp() {
  ...
  const bubbleText = (b) => {
    if (b.completed) return "うそ…\nやられたぁ…♡";
    if (b.hp > 2) return "ざぁこ♡ \n ざぁこ♡\n継続力よわよわ♡";
    if (b.hp === 1) return "や、やばっ……♡";
    return `あと\r\n${b.hp}つ`;
  };
  /* ---------- JSX ---------- */
  return (
    <div className="min-h-screen w-screen flex justify-center bg-gradient-to-br from-pink-500 via-red-300 to-yellow-100 py-10 text-[95%]">
            ...
            <div className="relative flex flex-col items-center mb-4">
              ...
              <div className="relative" ref={(el) => (enemyRefs.current[b.id] = el)}>
                  ...
                  <SpeechBubble
                     ...
                     size={120}
                     /* ↓ もう少し左へ & 上へずらす */
                      className="absolute -top-120 -left-16"
                  />

                <img
                  src={iconSrc(b)}
                  alt=""
                  className={`w-48 h-48 object-contain ${hit && hit.bid === b.id ? "animate-hitShake" : ""}`}
                />
      ...
      <style>{`
        ...
        @keyframes slash{0%{transform:rotate(10deg) translate(0,0);}
        ...
        @keyframes dmg{0%{transform:translate(-50%,0);opacity:1;}
          100%{transform:translate(-50%,-32px);opacity:0;}}
        .animate-dmg{animation:dmg .6s ease-out forwards;}

        @keyframes shakeSoft{0%,100%{transform:translate(0);}
        ...
        @keyframes hitShake{0%,100%{transform:translate(0);}
        ...
        @keyframes blast{0%{opacity:.7;transform:scale(1);}
